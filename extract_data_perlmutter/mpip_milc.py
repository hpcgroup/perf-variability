...
##############################################################################
# 2) Traverse directories to collect all runs, returning [(run_name, {mpi_calls: {call: time_s, ...}, app_time: T1, total_mpi_time: T2}), ...]
##############################################################################
...
def main():
    ...
    csv_path = os.path.join("MILC.csv")
    ...
    # --- Option 2: Load data directly from CSV (Example) ---
    # Comment out Option 1 and uncomment this section to load from CSV instead
    print("\nLoading data from CSV file...")
    csv_path_to_load = os.path.join(save_dir, "amg_64nodes_runs_data.csv")
    runs_sorted_by_apptime = load_runs_from_csv(csv_path_to_load)
    if not runs_sorted_by_apptime:
        print("Could not load data from CSV. Exiting.")
        return
    # Note: Data loaded from CSV is already sorted if saved correctly.
