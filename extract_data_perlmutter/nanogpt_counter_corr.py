...
# Assume root path
DATA_ROOT = Path("/pscratch/sd/c/cunyang/result/nanoGPT/64nodes")
...
def parse_nanogpt_out(nanogpt_file: Path):
    """
    Parse nanoGPT output file to extract iteration times and counter data.
...
def main():
    ...
    for subdir in DATA_ROOT.iterdir():
        ...
        # Find the nanoGPT.out file in this directory
        nanogpt_file = subdir / "nanoGPT.out"
        if not nanogpt_file.exists():
            continue

        time_list, counters_data =  parse_nanogpt_out(nanogpt_file)
    ...
    corr_df.to_csv("nanoGPT_mean.csv", index=False)
    ...
    corr_df.to_csv("nanoGPT_max.csv", index=False)
    ...
    corr_df.to_csv("nanoGPT_min.csv", index=False)

    # # To output results to CSV：
    df.to_csv("nanoGPT_counters_summary.csv", index=False)
