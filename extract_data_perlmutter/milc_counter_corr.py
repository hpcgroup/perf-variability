...
# Assume root path
DATA_ROOT = Path("/pscratch/sd/c/cunyang/result/MILC/64nodes")
...
def parse_milc_out(milc_file: Path):
    """
    """
    time_list = []
...
def main():
    ...
    for subdir in DATA_ROOT.iterdir():
        ...
        # Find the milc.out file in this directory
        milc_file = subdir / "milc.out"
        if not milc_file.exists():
            continue

        time_list, counters_data =  parse_milc_out(milc_file)
    ...
    if counter_cols: # Check if there are any mean columns before proceeding
        ...
        corr_df.to_csv("MILC2023_mean.csv", index=False)
    else:
    ...
    if counter_cols:
        ...
        corr_df.to_csv("MILC2023_max.csv", index=False)
    else:
    ...
    if counter_cols:
        ...
        corr_df.to_csv("MILC2023_min.csv", index=False)
    else:
    ...
    # df.to_csv("MILC_counters_full_data_with_derived.csv", index=False) # Example: Save full data
