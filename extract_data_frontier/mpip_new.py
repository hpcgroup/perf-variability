...
# app = "MILC"
nodes = 64
trace_dir = os.path.join("/lustre/orion/csc547/scratch/keshprad/perfvar", f'{app}_logs', f'{nodes}nodes')
save_dir = os.path.join("/lustre/orion/csc547/scratch/keshprad/perfvar", f'{app}_logs', f'{nodes}nodes')
    ...
    with open(file_path, 'r') as f:
        for line in f:
            ...
            if not in_section:
                ...
                total_time_s /= 512.0  # Divide by 256 (e.g., 256 GPUs)
...
def collect_runs(trace_dir):
    job_dirs = sorted(
        [d for d in os.listdir(trace_dir)
         ...
         and util.verify_app_dir(d, app, nodes)],
    )
    # job_dirs = [d for d in job_dirs if d != "2025-02-24_17-57-32-job36211897"]
    ...
    for d in job_dirs:
        ...
        mpi_files = list(glob.glob(os.path.join(subdir_path, "mpiP", "*.mpiP"))) + \
            list(glob.glob(os.path.join(subdir_path, "*.mpiP")))
        ...
        run_data = {
            ...
            'app_time': apptime_val / 512.0,
            'total_mpi_time': mpitime_val / 512.0 # Note: This mpi_time might differ slightly from sum(mpi_call_dict.values())
        }
...
def collect_app_mpi_data(trace_dir):
    ...
    for d in job_dirs:
        ...
        if apptime_val is not None and mpitime_val is not None:
            ...
            app_times.append(apptime_val / 512.0)
            mpi_times.append(mpitime_val / 512.0)
...
def plot_six_stacked_bars(avg_data, best_data, q1_data, q2_data, q3_data, worst_data, k=7):
    ...
    ax.set_title(f"{app} 64nodes Runtime Breakdown (Compute + MPI)")
    ...
    plt.savefig(os.path.join(save_dir, f"{app}_mpi_compute_stacked_6_comp_first_asc_braces.png")) # Added _braces
    plt.savefig(os.path.join(save_dir, f"{app}_mpi_compute_stacked_6_comp_first_asc_braces.pdf")) # Added _braces
...
def plot_routine_distribution(runs, routine1_name, range_bins1, routine2_name, range_bins2, num_bins=5):
    ...
    # --- Final adjustments ---
    fig.suptitle(f"Distribution of MPI {routine1_name} and {routine2_name} Times ({app})", fontsize=16)
    plt.tight_layout(rect=[0, 0.03, 1, 0.95]) # Adjust layout to prevent title overlap

    # Update filename
    filename_base = f"{app}_routine_distribution_{routine1_name}_{routine2_name}_categorical"
...
def main():
    ...
    csv_path = os.path.join(save_dir, f"{app}.csv")
